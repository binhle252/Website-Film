...
const deleteMovie = (req, res) => {
    ...
    // Bắt đầu xóa từ bảng phụ → bảng chính
    const deleteRelatedQueries = [
        { sql: 'DELETE FROM movie_categories WHERE movie_id = ?', label: 'movie_categories' },
        { sql: 'DELETE FROM episodes WHERE movie_id = ?', label: 'episodes' },
        { sql: 'DELETE FROM reviews WHERE movie_id = ?', label: 'reviews' }, // nếu có bảng reviews
        // thêm bảng phụ khác nếu cần
    ];

    // Thực hiện từng truy vấn xóa
    const deleteNext = (index) => {
        if (index >= deleteRelatedQueries.length) {
            // Sau khi xóa bảng phụ, xóa chính phim
            const deleteMovieSql = 'DELETE FROM movies WHERE movie_id = ?';
            db.query(deleteMovieSql, [movieId], (err, result) => {
                if (err) {
        ...
        const { sql, label } = deleteRelatedQueries[index];
        db.query(sql, [movieId], (err) => {
            if (err) {
                console.error(`Lỗi khi xóa dữ liệu từ bảng ${label}:`, err);
                return res.status(500).json({ message: `Lỗi khi xóa dữ liệu liên kết (${label})`, error: err.message });
            }
            deleteNext(index + 1);
        });
    };

    deleteNext(0); // Bắt đầu từ truy vấn đầu tiên
};
