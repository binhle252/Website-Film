...
const addEpisode = (req, res) => {
    ...
    // ✅ Dùng callback, không dùng await
    const checkQuery = `SELECT * FROM episodes WHERE movie_id = ? AND episode_number = ?`;
    db.query(checkQuery, [movie_id, episode_number], (err, results) => {
        if (err) {
            console.error("Lỗi truy vấn kiểm tra:", err);
            return res.status(500).json({ error: 'Lỗi kiểm tra tập phim.' });
        }

        if (results.length > 0) {
            return res.status(400).json({ error: 'Số tập này đã tồn tại!' });
        }
        ...
        db.query(insertQuery, [movie_id, episode_number, title, video_url], (err, result) => {
            if (err) {
                console.error("Lỗi khi thêm tập:", err);
                return res.status(500).json({ error: 'Lỗi thêm tập phim.' });
            }

            return res.status(201).json({ message: 'Thêm tập phim thành công!', episode_id: result.insertId });
        });
    });
};
